...
switch answer
    ...
    case '2nd'
        ...
        epoch       = [-0.400 -1.250];
        thresholdMax   = 120;
        train.channel = {1:128;  62:102; 30:62};
    ...
    for dataLoop = 1:3
        ...
        for trialLoop = 1:size(datall,3)
            ...
            for spatialLoop = 1:3
                DWTFeat = [];
                % Spatial filtering
                %[~,spatDat,~, ~] = LDAbeamformer(train.score{1,spatialLoop}, dat(train.channel{spatialLoop},:));
                spatDat = diySpatialFilter(train.score{1,spatialLoop}, dat(train.channel{spatialLoop},:));    
                
%                 repvec = repmat([2 1 0], 1, length(mat)/3);
%                 figure((q+repvec(q))/3)
%                 if any(q == [1:3:length(mat)-2]) && dataLoop == 1
%                     subplot(4,1,1)
%                     plot(spatDat)
%                     hold on;
%                 elseif any(q == [2:3:length(mat)-1]) && dataLoop == 1
%                     subplot(4,1,2)
%                     plot(spatDat)
%                     hold on;
%                 elseif any(q == [3:3:length(mat)]) && dataLoop == 1
%                     subplot(4,1,3)
%                     plot(spatDat)
%                     hold on;
%                 elseif dataLoop == 3
...
%                 end
                ...
                % get ERP epoch
                erp_epoch_beam = spatDat(:, (feat.offsetSamp + feat.peakEndSamp)+1 : (feat.offsetSamp + feat.peakBegSamp));       
                ...
                if spatialLoop == 3 %% Difference between Occ Electrodes and Reference Electrodes
                    ...
                    %[~,spatDat,~, ~] = LDAbeamformer(train.score{1,2}, dat(train.channel{2,1},:));
                    spatDat = diySpatialFilter(train.score{1,2}, dat(train.channel{2, 1},:));
                
                    for DWTLoop = 1:4
                        DWTspatDat = [spatDat cfg.padding(1,:)]; 
                        [c,l] = wavedec(DWTspatDat,6,char(DWTname(DWTLoop)));
                        [c,l] = wavedec(spatDat,6,char(DWTname(DWTLoop)));
                        Delta = appcoef(c,l,char(DWTname(DWTLoop)));
                        [BetaGamma,Alpha,Theta] = detcoef(c,l,[4 5 6]);
                        DWTFreq = {Delta(2:17) Theta(2:16) Alpha(2:29) BetaGamma(2:55)};
                        DWTFeatTemp = [];

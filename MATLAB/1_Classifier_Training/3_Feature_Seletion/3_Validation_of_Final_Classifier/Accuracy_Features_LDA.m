...
partitionedModel = crossval(classificationDiscriminant, 'CVPartition', cp);
...
[lable, results] = predict(classificationDiscriminant, predictors);
...
for j = 1:length(table2array(predictors))
    tic;
    results = predict(classificationDiscriminant, predictors(j,:));
    timeClass(j) = toc;
...
figure(9)
figure('Color','white')
pspectrum(mean(loom{1},2),500,'spectrogram','TimeResolution',0.9, 'FrequencyLimits',[2 25])
caxis([-50 30])
colormap jet
figure(10)
figure('Color','white')
pspectrum(mean(random{1},2),500,'spectrogram','TimeResolution',0.9, 'FrequencyLimits',[2 25])
caxis([-50 30])
colormap jet
figure(11)
figure('Color','white')
pspectrum(mean(loom{2},2),500,'spectrogram','TimeResolution',0.9, 'FrequencyLimits',[2 25])
caxis([-50 30])
colormap jet
figure(12)
figure('Color','white')
pspectrum(mean(random{2},2),500,'spectrogram','TimeResolution',0.9, 'FrequencyLimits',[2 25])
caxis([-50 30])
colormap jet
